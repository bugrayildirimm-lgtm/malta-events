...
const guidePages = {
  ...
  'guide/malta-nightlife-guide': {
    ...
    content: `
      ...
      <p>Malta's festival scene goes well beyond club nights. <a href="/event/earth-garden-festival-2026-3564">Earth Garden</a> (May) is the island's biggest alternative festival with five stages at Ta' Qali National Park — think world music, techno, reggae, and psytrance across three days with 30,000 attendees. Bloom Festival brings slick house and techno production to outdoor venues with international headliners. <a href="/event/dark-malta-festival-2026-2026">Dark Malta Festival</a> (April) is a unique gothic and industrial music event attracting alternative culture fans from across Europe. And the Isle of MTV (summer) is completely free, drawing tens of thousands to Floriana for performances by global pop and dance acts. Check our <a href="/festivals-malta">Festivals page</a> for all upcoming dates.</p>

      <h2>Big Concerts Coming to Malta in 2026</h2>
      <p>Malta is increasingly attracting major international artists. <a href="/event/scorpions-live-in-concert-4792">Scorpions</a> are bringing their legendary rock show to Malta — a rare opportunity to see one of the biggest rock bands in history on the island. <a href="/event/pitbull-im-back-with-special-guest-lil-jon-2486">Pitbull with special guest Lil Jon</a> is also set to perform as part of his "I'm Back" tour, promising an explosive night of hits. These headline concerts usually sell out fast so grab tickets early. Browse our <a href="/music-events-malta">Music Events page</a> for all upcoming concerts.</p>
  ...
  'guide/things-to-do-malta-tourists': {
    ...
    content: `
      ...
      <h2>Major Concerts in Malta 2026</h2>
      <p>Malta is becoming a serious stop for world-touring artists. In 2026, rock legends <a href="/event/scorpions-live-in-concert-4792">Scorpions</a> are performing live in Malta — a bucket-list moment for any rock fan. Global hitmaker <a href="/event/pitbull-im-back-with-special-guest-lil-jon-2486">Pitbull is also coming with Lil Jon</a> as part of his "I'm Back" world tour. These big-ticket concerts typically sell out weeks in advance, so if your visit coincides with one, book early. Check our <a href="/music-events-malta">Music Events page</a> for the full concert calendar.</p>
...
<body>
  ...
  <section style="max-width:900px;margin:60px auto 0;padding:0 20px;color:#334155;font-size:0.95rem;line-height:1.8">
    ...
    <p>Malta has a packed calendar throughout the year. During carnival season in February you will find parades in Valletta and Nadur, while spring brings Holy Week processions and the Malta International Fireworks Festival. Late spring features <a href="/event/earth-garden-festival-2026-3564" style="color:#FF385C;text-decoration:underline">Earth Garden Festival</a>, Malta's largest alternative music festival at Ta' Qali National Park with 30,000 attendees across five stages. Summer is peak season with the Isle of MTV, Bloom Festival, BeerFest, and numerous boat parties and pool events. <a href="/event/dark-malta-festival-2026-2026" style="color:#FF385C;text-decoration:underline">Dark Malta Festival</a> in April draws alternative and gothic music fans from across Europe. Autumn features Notte Bianca and BirguFest, and winter brings the Malta Christmas markets, New Year celebrations, and the Baroque Festival at some of the island's most historic venues.</p>

    <h3 style="font-size:1.15rem;font-weight:700;color:#0f172a;margin:30px 0 10px">Big Concerts Coming to Malta</h3>
    <p>Malta is attracting world-class artists in 2026. Rock legends <a href="/event/scorpions-live-in-concert-4792" style="color:#FF385C;text-decoration:underline">Scorpions</a> are performing live — a once-in-a-lifetime opportunity on the island. <a href="/event/pitbull-im-back-with-special-guest-lil-jon-2486" style="color:#FF385C;text-decoration:underline">Pitbull with special guest Lil Jon</a> is also headed to Malta as part of his worldwide "I'm Back" tour. These headline shows sell out fast, so check our <a href="/music-events-malta" style="color:#FF385C;text-decoration:underline">Music Events page</a> for tickets and dates.</p>
...
app.post('/admin/api/send-newsletter', async (req, res) => {
  ...
  try {
    ...
    const upcoming = evResult.rows.filter(e => {
      const sd = getStartDate(e.event_date);
      return sd && sd >= now;
    }).sort((a,b) => {
      const da = getStartDate(a.event_date);
      const db = getStartDate(b.event_date);
    ...
    const transporter = nodemailer.createTransport({
      ...
      port: 587,
      auth: { user: BREVO_SENDER, pass: BREVO_SMTP_KEY }
    });
    
    let sent = 0;
    let failed = 0;
    const emailSubject = subject || 'This Week in Malta — Malta Event Guide';
    
    for (const sub of subs.rows) {
      try {
        const personalHtml = emailHtml.replace(/%%EMAIL%%/g, encodeURIComponent(sub.email));
        await transporter.sendMail({
          from: '"Malta Event Guide" <' + BREVO_SENDER + '>',
          to: sub.email,
        ...
        failed++;
        console.log('[Newsletter] Failed to send to', sub.email, e.message);
      }
    }
...
app.get('/admin/api/newsletter-preview', async (req, res) => {
  if (!authCheck(req, res)) return;
  try {
    const evResult = await pool.query("SELECT * FROM events WHERE COALESCE(status,'live')='live' ORDER BY id DESC LIMIT 200");
    const now = new Date();
    ...
    res.json({ events: upcoming.map(e => ({ title: e.title, date: e.event_date, location: e.location, category: e.category, image: e.image_url })), subscribers: subs.rows[0].c });
  } catch(e) { res.json({ events: [], subscribers: 0 }); }
});
      ...
      <div class="analytics">
        ...
        <div style="margin-top:20px">
          <div style="background:#1e293b;padding:18px;border-radius:10px">
            <h3 style="margin:0 0 15px;font-size:1rem;color:white">📧 Send Weekly Newsletter</h3>
            <div style="margin-bottom:12px">
              <label style="color:#94a3b8;font-size:0.8rem;display:block;margin-bottom:4px">Subject Line</label>
              <input type="text" id="nlSubject" value="This Week in Malta — Malta Event Guide" style="width:100%;padding:10px 14px;border-radius:8px;border:1px solid #334155;background:#0f172a;color:white;font-family:inherit;font-size:0.9rem;box-sizing:border-box">
            </div>
            <div style="margin-bottom:12px">
              <label style="color:#94a3b8;font-size:0.8rem;display:block;margin-bottom:4px">Preview Text (shown in email preview)</label>
              <input type="text" id="nlPreview" value="The best events happening in Malta this week" style="width:100%;padding:10px 14px;border-radius:8px;border:1px solid #334155;background:#0f172a;color:white;font-family:inherit;font-size:0.9rem;box-sizing:border-box">
            </div>
            <div id="nlPreviewBox" style="background:#0f172a;border-radius:8px;padding:14px;margin-bottom:12px;display:none">
              <div style="color:#64748b;font-size:0.75rem;margin-bottom:8px">PREVIEW — Top 8 events that will be included:</div>
              <div id="nlEventList" style="font-size:0.85rem;color:#e2e8f0"></div>
            </div>
            <div style="display:flex;gap:10px">
              <button onclick="previewNewsletter()" style="padding:10px 20px;border-radius:8px;border:1px solid #334155;background:transparent;color:#94a3b8;font-family:inherit;font-size:0.85rem;cursor:pointer;font-weight:600">👁️ Preview</button>
              <button onclick="sendNewsletter()" id="nlSendBtn" style="padding:10px 24px;border-radius:8px;border:none;background:#22c55e;color:white;font-family:inherit;font-size:0.85rem;cursor:pointer;font-weight:700">📧 Send Newsletter</button>
            </div>
            <div id="nlStatus" style="margin-top:10px;font-size:0.85rem;display:none"></div>
          </div>
        </div>
      </div>
    </div>
...
function previewNewsletter(){
  fetch('/admin/api/newsletter-preview',{headers:{Authorization:auth}})
    .then(function(r){return r.json()})
    .then(function(d){
      var box=document.getElementById('nlPreviewBox');
      box.style.display='block';
      var html='<div style="color:#94a3b8;font-size:0.75rem;margin-bottom:6px">Sending to '+d.subscribers+' subscribers</div>';
      d.events.forEach(function(e){
        html+='<div style="padding:6px 0;border-bottom:1px solid #1e293b;display:flex;gap:8px;align-items:center">';
        if(e.image)html+='<img src="'+e.image+'" style="width:40px;height:40px;border-radius:6px;object-fit:cover">';
        html+='<div><div style="font-weight:600;font-size:0.85rem">'+e.title+'</div><div style="color:#64748b;font-size:0.75rem">📅 '+e.date+' · 📍 '+(e.location||'Malta')+'</div></div></div>';
      });
      document.getElementById('nlEventList').innerHTML=html;
    });
}
function sendNewsletter(){
  if(!confirm('Send newsletter to all subscribers now?'))return;
  var btn=document.getElementById('nlSendBtn');
  var status=document.getElementById('nlStatus');
  btn.disabled=true;btn.textContent='Sending...';btn.style.opacity='0.5';
  status.style.display='block';status.style.color='#94a3b8';status.textContent='Sending emails, please wait...';
  fetch('/admin/api/send-newsletter',{
    method:'POST',
    headers:{Authorization:auth,'Content-Type':'application/json'},
    body:JSON.stringify({
      subject:document.getElementById('nlSubject').value,
      previewText:document.getElementById('nlPreview').value
    })
  }).then(function(r){return r.json()})
    .then(function(d){
      btn.disabled=false;btn.textContent='📧 Send Newsletter';btn.style.opacity='1';
      if(d.ok){
        status.style.color='#4ade80';
        status.textContent='✅ Sent to '+d.sent+' subscribers'+(d.failed?' ('+d.failed+' failed)':'');
      }else{
